...
double atomicRadius = 2.1*pow(10,-10); //Covalent radius of atom [m]
double vacFormationEnergy = 2.7237*pow(10,-19); //Energy per vacancy formation [Joules]

//double b = 0.1371*pow(10,-9); //Burger's vector [m]
double b = 0.25*pow(10,-9); //Burger's vector [m]
double G = 161*pow(10,9); //Shear modulus in [Pa]
double poissonRatio = 0.28; //Poisson's Ratio
double Va = 10*pow((0.316 * pow(10,-9)),3); //Activation Volume
double Eo = 9.6*pow(10,-20); //Migration Energy [Joules]
...
int PrintStatsFreq = 400; //Frequency of outputting the test statistics
int PrintEVLFreq = 400;
...
int outputEVL=1; //1 or 0 for printing evl_#.txt files -- printed in "evl1" folder
...
////////For Periodic Stress calculations////////
int isPerioidc = 0; //0=do not use periodic stress calcualtion, 1 = use periodic stress calculation
int numImages = 5; //Number of images of the dislocation segments to consider 
...
int constVacNumber = 30; //Number of vacancies to consider for each trial of the parametric stdy IF dynamicBoxResizing==1
...
int recountFreq = 200; //If requireConstantVacs==1, check the number of vacancies every recountFreq iterations and add/remove the required number of vacancies

int trials = 3; //Number of trials to do within the min and max bounds. For example, if minTemp=100, maxTemp=200, and trials = 3, simulations would be run at 100, 150, and 300.
...
double maxTemp = 2000; //Maximum temperature to use in parametric study [Kelvins]

//double minVacConcentration = 1e20; //Minimum vacancy concentration to use in parametric study [vacs/m^3]
double maxVacConcentration = 5; //Maximum vacancy concentration to use in parametric study - must be a multiplier of the thermal vacancy concentration[vacs/m^3]
...
double minPressure = pow(10,-22); //Minimum stress [pressure] to use in parametric study[Pa]
double maxPressure = 5*pow(10,-22);  //Maximum stress [pressure] to use in parametric study[Pa]
