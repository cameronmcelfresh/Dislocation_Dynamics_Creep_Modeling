...
int main (int argc, char* argv[])
{
	...
	if(useParametricStudy==1)
	{
		...
		for(int iTemp =0; iTemp<tempTrials+1; iTemp++) //Temperature variation loop
		{

			Temp = minTemp + ((maxTemp-minTemp)/tempTrials)*iTemp; //Temperature to use during this trial
			...
			if(Temp>1200)
				{
				double newNsteps = 750100;
				rewriteDD(newNsteps);
				}

				
			for(int iVacConc= 0; iVacConc<concTrials+1; iVacConc++) //Vacancy concentration variation loop
			{
				

				if(useStress==1) //Only enter the stress variation loops if there is applied stress
				{
					for(int iStress = 0; iStress<stressTrials+1; iStress++) //sigma_xx stress variation loop
					{

						int TotalTrials = pow(trials,3)*repeatTrials; //Total number of trials that will be run

						appliedPressure = minPressure + ((maxPressure-minPressure)/stressTrials)*iStress; //sigma_xx to use during this trial
						...
						vacancyConcentration = minVacConcentration*thermalVacancyConcentration + thermalVacancyConcentration*((maxVacConcentration-minVacConcentration)/concTrials)*iVacConc; //Vacancy concentration to use during this trial
						...
						{	
							...
							if(DDSimulationText==0)
								...
								std::cout<<"T = " << Temp << " [K] , Vac Concentration = " << vacancyConcentration << " [vacs/m^3] , " << "Pressure = " << appliedPressure << " [Pa] || "; 
				...
				else //Skip the stress variations if there is no applied stress
				{
					for(int repeatCycles=0; repeatCycles<repeatTrials; repeatCycles++) //Repeat the simulations however many times desired
						...
						if(DDSimulationText==0)
							{
							std::cout.rdbuf(cout_sbuf); // restore the original stream buffer
							std::cout<<"T = " << Temp << " [K] , Vac Concentration = " << vacancyConcentration << " [vacs/m^3] || ";
